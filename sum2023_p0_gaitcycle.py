...
class botControlFC(JoyApp):
    ...
    def onStart(self):
        if self.robot is not None:
            ...
            self.planR = GaitCyclePlan( self, self.turnR, maxFreq = 0.3, Nx10='Nx10/@set_pos', Nx04='Nx04/@set_pos')
        else:
            self.plan = GaitCyclePlan( self, self.moveFwd, maxFreq = 0.3,
            x='#catX', y='#catY')
        #Curry function creates a callback using it's arguements
        #self.planfwd.onStart = curry(progress,">>> START")
        #self.planfwd.onStop = curry(progress,">>> STOP")
        #self.planfwd.setFrequency(0.2)
        #self.moveForward.start()
        self.planR.onStart = curry(progress,">>> START")
        self.planR.onStop = curry(progress,">>> STOP")
        self.planR.setFrequency(0.2)

        #self.planL.onStart = curry(progress,">>> START")
        #self.planL.onStop = curry(progress,">>> STOP")
        #self.planL.setFrequency(0.2)
